...
def run_nosqli_test(engine, point, param, level, bypass, waf_name=None):
    ...
    if bypass:
        forced_technique = bypass_oracle.probe_and_get_technique(engine, url, method, base, param, 'nosqli', waf_name)

    for pay in payloads:
        final_pay = apply_tampering(pay, 'nosqli', bypass, waf_name, technique=forced_technique)

        if bypass:
            resp = engine._send_request_raw(url, method, base, param, final_pay)
        else:
        ...
        if resp is not None:
            for err in NOSQL_ERRORS:
                if err in resp.text:
                    details = f"Erreur DB: {err}"
                    if bypass and forced_technique:
                        details += f" - Technique de bypass utilisée : {forced_technique}"
                    engine.add_vulnerability("NoSQLi (Error)", url, final_pay, details, parameter=param)
                    break

            if len(resp.text) > (baseline_len + 50):
                if "[{" in resp.text and "}]" in resp.text:
                    details = f"Dump JSON (+{len(resp.text)-baseline_len}o)"
                    if bypass and forced_technique:
                        details += f" - Technique de bypass utilisée : {forced_technique}"
                    engine.add_vulnerability("NoSQLi (Boolean/Dump)", url, final_pay, details, parameter=param)
